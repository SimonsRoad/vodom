...
for i = 1:num_iterations
    ...
    landmark_sample = landmark_sample(1:3,:);   %BUGFIX for P3P
    ...
    poses = p3p(landmark_sample, normalized_bearings);  % BUG: landmark_sample should be 3x3
    % Decode p3p output
    ...
    projected_points = projectPoints(...
        (R_C_W_guess(:,:,1) * X(1:3,:)) + ...   %BUGFIX in X
    ...
    projected_points = projectPoints(...
        (R_C_W_guess(:,:,2) * X(1:3,:)) + ...   %BUGFIX in X
    ...
    if num_inlier > max_num_inliers && num_inlier >= 10%min_inlier_count BUG: 30 Inlier is waay too much. We only have ~15-20 matches!
...
if max_num_inliers == 0
    ...
    X_3 = X(1:3,:);
    M_C_W = dlt(P(:, best_inlier_mask>0)', X_3(:, best_inlier_mask>0)', K); %Debug
    R_CW = M_C_W(:, 1:3);
    t3_CW = M_C_W(:, end);%/t3norm; %Debug
end
