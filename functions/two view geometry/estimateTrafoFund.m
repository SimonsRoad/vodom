function [R, t3, X, P, Pdb, t3norm] = estimateTrafoFund(...
                                            kp0, kp1, K, t3norm, num_iter)
...
[F, inliers] = estimateFundamentalMatrix(fliplr(kp0'), fliplr(kp1'), ...
                              'Method', 'RANSAC', ...
                              'DistanceThreshold', 10.0, 'NumTrials', 2000);
p0 = p0(:,inliers'); 
p1 = p1(:,inliers'); 
E = K'*F*K;
...
M1 = K * eye(3,4);
M2 = K * [R, t3];
X = linearTriangulation(p0,p1,M1,M2);
P = kp1(:,inliers'); 
Pdb = kp0(:,inliers'); 
end