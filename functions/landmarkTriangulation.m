...
p1 = [candidate_keypoints_1; ones(1,num_points)]; %db
p2 = [candidate_keypoints; ones(1,num_points)];%q

current_pose_inv = invertPose(current_pose);    %T_CW
...
for j=1:num_points  % Triangulate each candidate pt.
...
% Output
new_keypoints = candidate_keypoints(:,new); % Only the candidate keypts that passed all tests (reliable angle, low discard counter, not behind camera, 3D->2D reprojection correct.
new_landmarks = P(:,new);
updated_candidate_keypoints = candidate_keypoints(:,~new);  % All candidate keypts that failed the tests
